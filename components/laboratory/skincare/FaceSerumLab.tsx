...
export default function FaceSerumLab() {
...
"Silky Texture",
"Crystal Serum",
"Velvet Molecular Serum",
"Ultra Repair Formula",
]
...
const synergyBonus =
selectedActives.includes("Hyaluronic Acid")
&&
selectedActives.includes("Niacinamide")

const glowBoost =
selectedActives.includes("Vitamin C")
&&
selectedActives.includes("Peptides")
const bottleColor =
selectedConcerns.includes("Hydration")
? "from-cyan-200/40 to-blue-500/20"

:selectedConcerns.includes("Anti Aging")
? "from-amber-200/40 to-yellow-500/20"

:selectedConcerns.includes("Dark Circles")
? "from-violet-200/40 to-purple-500/20"
...
const liquidColor =
selectedConcerns.includes("Hydration")
? "from-cyan-200/40 to-blue-400/30"

:selectedConcerns.includes("Anti Aging")
? "from-amber-200/40 to-orange-500/30"

:selectedConcerns.includes("Dark Circles")
? "from-purple-200/40 to-violet-500/30"
...
window.print()
  ...
  const compatibleActives =
  ingredients.naturalActives.filter(
    (item) =>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="mt-8 text-white/60 max-w-3xl mx-auto text-lg leading-relaxed">
              Engineer a concentrated skincare formula powered by
              active ingredients and AI formulation science.
            </p>
...
<div className="mt-20">
  ...
  <h2 className="text-3xl font-serif mb-8">
    02 — Select Active Ingredients
  </h2>

<div className="grid md:grid-cols-3 gap-6">

{compatibleActives.map((active) => {

const selected =
selectedActives.includes(active.name)
...
onClick={() => toggleActive(active.name)}
    ...
    {textures.map((item)=>(
      ...
      >
        ...
        <p className="mt-4 text-sm text-white/50">
          AI-designed sensory texture experience.
        </p>
...
>
  ...
  <div className="space-y-6">
    ...
    {selectedActives.map((ingredient, index) => {
...
{texture}
  ...
  {generatedTexture}
    ...
    <div className="mt-16">
      ...
      <div className="flex flex-wrap gap-4">
        {selectedActives.map((ingredient) => (
          <div
            key={ingredient}
  ...
  {selectedActives.includes("Tea Tree Oil") && (
    <li>• Botanical purification technology</li>
  )}

  {selectedActives.includes("Green Tea Extract") && (
    <li>• Antioxidant environmental protection</li>
  )}

  {selectedActives.includes("Chamomile") && (
    <li>• Sensitive skin calming effect</li>
  )}

  {selectedActives.includes("Rose Water") && (
    <li>• Luxury floral skin nourishment</li>
  )}

  {selectedActives.includes("Aloe Vera") && (
...
className="mt-24 rounded-[40px] border border-gold/20 p-12 bg-white/5"
>
    ...
    />
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-2">
          {selectedActives.slice(0, 3).map((ingredient) => (
            <span
              key={ingredient}
...
<div className="mt-12">
  ...
  <h3 className="text-5xl font-serif text-gold">
    {compatibilityScore}%
  </h3>
...
</div>{/* Detailed Formula Report */}

<div className="mt-24">

<h3 className="text-3xl font-serif mb-10">

Detailed Formula Report
...
<span className="text-white/40 uppercase text-xs">
...
<div
key={item}
className="px-4 py-2 rounded-full bg-gold/10 border border-gold/20"
>
...
<h4 className="mt-6 text-2xl font-serif">
...
This serum combines hydration balance,
advanced ingredient penetration,
luxury sensory formulation and
...
<button
onClick={downloadFormula}
className="px-10 py-5 rounded-full bg-gold text-black hover:scale-105 transition-transform"
>
